...
module IPTI
  module Client
    class BayController < IPTI::Controller
      attr_accessor :d4_modules, :lp_modules, :oc_modules, :lt_modules
      ...
      message_type :lt_display,          :code => "39",
                                         :response_handler => :lt_display_text,
                                         :formatter        => :format_lt_display_text
      ...
      message_type :set_num_of_oc,       :code => "84",
                                         :response_handler => :get_oc_modules_84,
                                         :formatter        => :format_oc_modules_84,
                                         :ack_handler      => :ack_oc_modules
      ...
      def initialize(address, connection)
        ...
        @seq = 0
        @d4_modules, @d4_list, @d4_starting_addr = {}, [], "01"
        @lp_modules, @lp_list, @lp_starting_addr = {}, [], "01"
        @oc_modules, @oc_list, @oc_starting_addr = {}, [], "01"
        @lt_modules, @lt_list, @lt_starting_addr = {}, [], "01"
        ...
        unless  @bay_model.nil?
          ...
          @bay_model.devices.each do |device|
             case device
               ...
               when LtModuleModel then
                 @lt_list << device
                 @lt_modules[device.address] = device
             end
          end
        end
        ...
        @lt_starting_addr = @lt_list.map(&:address).min unless @lt_list.empty?
      ...
      def lt_display_text(message)
        lt_msg = IPTI::PickMaxMessage.new(self, self.message_types[:lt_display])
        if message.bytes.size > 10
          lt_addr = message.bytes.slice(7,2)
          lt_module = @lt_modules[lt_addr]
          if lt_module.nil?
            lt_msg.fields[:success] = false
          else
            lt_msg.fields[:success] = true
            lt_module.push_msg(message)
          end
          lt_msg.fields[:ack] = true
          lt_msg.sequence = message.sequence
        end
        lt_msg
      end

      def format_lt_display_text(args={})
        return "'" if args.empty?
        args[:success] ? "" : "ER"
      end
